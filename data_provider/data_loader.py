...
class Dataset_WellsColumns(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 ...
                 step_len=None):
        """
        井产量数据集 - 使用滑动窗口划分训练/测试样本
        
        Args:
            size: [input_len, label_len, output_len]
                - input_len: 输入序列长度（固定）
                - label_len: 标签起始长度（通常等于output_len）
                - output_len: 输出序列长度（固定）
            step_len: 滑动窗口步长，默认为output_len（无重叠）
        """
        if size is None:
            raise ValueError('size must be provided as [input_len, label_len, output_len]')
        self.input_len = size[0]  # 固定输入长度
        self.label_len = size[1]   # 标签长度（通常等于output_len）
        self.output_len = size[2]  # 固定输出长度
        self.step_len = step_len if step_len is not None else self.output_len  # 默认步长=输出长度（无重叠）
    ...
    def _read_and_prepare(self):
        ...
        series_all = []
        min_window_size = self.input_len + self.output_len
        ...
        if len(series_all) == 0:
            raise ValueError(f'No well has enough length (>={min_window_size}) for window generation.')

        # 按井划分train/val/test: 前70%井训练，10%验证，20%测试
        num_wells = len(series_all)
        train_split = max(1, int(num_wells * 0.7))
        val_split = max(train_split + 1, int(num_wells * 0.8))
        
        # 全局Scaler（基于训练井的全部数据）
        self.scaler = StandardScaler()
        if self.scale:
            train_wells = series_all[:train_split]
            train_concat = np.concatenate(train_wells, axis=0)
        ...
        if self.set_type == 0:  # train
            self.well_series = series_all[:train_split]
        elif self.set_type == 1:  # val
            self.well_series = series_all[train_split:val_split]
        else:  # test
            self.well_series = series_all[val_split:]
        ...
        for well_idx, well_data in enumerate(self.well_series):
            ...
            # 计算该井可以生成多少个窗口
            num_windows = (well_length - min_window_size) // self.step_len + 1
            
            for window_idx in range(num_windows):
                start_idx = window_idx * self.step_len
                end_idx = start_idx + self.input_len + self.output_len
                
                # 确保不超出井的长度
                if end_idx <= well_length:
                    self.index_map.append((well_idx, start_idx))
                    total_windows += 1
        
        print(f'Generated {len(self.index_map)} sliding window samples from {len(self.well_series)} wells '
              f'(input_len={self.input_len}, output_len={self.output_len}, step={self.step_len})')
    ...
    def __getitem__(self, index):
        well_idx, start_idx = self.index_map[index]
        well_data = self.well_series[well_idx]
        
        # 提取固定长度的输入和输出窗口
        input_end = start_idx + self.input_len
        output_end = input_end + self.output_len
        
        # X: 输入序列 [start_idx : start_idx + input_len]
        seq_x = well_data[start_idx:input_end].copy()
        
        # Y: 输出序列 [start_idx + input_len : start_idx + input_len + output_len]
        # 包含label_len + pred_len部分
        seq_y = well_data[input_end:output_end].copy()
        
        # 标准化
        seq_x = self._transform(seq_x.astype(np.float32)).reshape(-1, 1)
        seq_y = self._transform(seq_y.astype(np.float32)).reshape(-1, 1)
        
        # 时间特征占位（freq='d' 需要3维: month/day/weekday）
        seq_x_mark = torch.zeros((self.input_len, 3))
        seq_y_mark = torch.zeros((self.output_len, 3))
